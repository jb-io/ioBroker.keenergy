...
class KebaMiddleware {
    ...
    async writeState(id, value) {
        const itemDefinition = await this.getStateConfiguration(id);
        if (!itemDefinition.writable) {
            throw new Error(`State ${id} is not writable.`);
        }
        await this.keba.writeVar(itemDefinition.native.name, this.#denormalizeStateValue(value, itemDefinition.native.type));
    }
