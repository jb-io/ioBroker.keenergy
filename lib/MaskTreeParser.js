...
class MaskTreeParser {
    ...
    async checkNodeVisible(node, context) {
        ...
        const visibleVar = this.replacePlaceholder(node.getAttribute('visibleVar'), context);
        if (visibleVar.includes(';')) {
            globalUtils.log.warn(`rejected check for ${visibleVar}`); // TODO Implement check for multiple vars
            return false;
        }
    ...
    getNodeAttribute(node, attribute, defaultValue) {
        ...
        switch (node.nodeName) {
            ...
            case 'var':
                ...
                this._readWriteVars[varName] = {
                    ...
                    writable: node.hasAttribute('readOnly') && node.getAttribute('readOnly') === 'false' && (!node.hasAttribute('inputLevel') || parseInt(node.getAttribute('inputLevel')) <= this._maxInputLevel),
